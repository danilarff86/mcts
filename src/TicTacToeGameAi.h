...
struct TicTacToeGameAI
{
    ...
    MovePosition get_ai_move( ) const;

private:
    static TicTacToeState::Board create_small_board( const AvailableCells& available );
    static TicTacToeBigGameState::BigBoard create_big_board( const AvailableCells& available );
    
    void make_move( MctsNodePtr node );
